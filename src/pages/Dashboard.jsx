...
import { Target, Flame, Dumbbell, DollarSign, TrendingUp, BookOpen, Zap } from "lucide-react";
...
const weekData = [
  { day: "Mon", goals: 80, calories: 2600, protein: 160 },
  { day: "Tue", goals: 60, calories: 2800, protein: 175 },
  { day: "Wed", goals: 100, calories: 2400, protein: 190 },
  { day: "Thu", goals: 75, calories: 2900, protein: 170 },
  { day: "Fri", goals: 90, calories: 2700, protein: 185 },
  { day: "Sat", goals: 50, calories: 3100, protein: 150 },
  { day: "Sun", goals: 70, calories: 2500, protein: 165 },
];
...
export default function Dashboard() {
  ...
  const avgSkill = skills.length > 0
    ? Math.round(skills.reduce((s, k) => s + k.progress, 0) / skills.length)
    : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={Target} label="Goals Today" color="bg-indigo-600"
          value={`${goalsCompleted}/${goalsTotal}`}
          sub="completed"
          progress={goalProgress}
        />
        <StatCard
          icon={Flame} label="Calories" color="bg-orange-600"
          value={todayCalories.toLocaleString()}
          sub={`target: ${nutritionTargets.calories.toLocaleString()}`}
          progress={calorieProgress}
        />
        <StatCard
          icon={Zap} label="Protein" color="bg-green-600"
          value={`${todayProtein}g`}
          sub={`target: ${nutritionTargets.protein}g`}
          progress={proteinProgress}
        />
        <StatCard
          icon={Dumbbell} label="Workout" color="bg-purple-600"
        ...
        />
        <StatCard
          icon={DollarSign} label="Spent Today" color="bg-red-600"
          value={`$${todaySpend.toFixed(2)}`}
          sub="credit card"
        />
        <StatCard
          icon={DollarSign} label="My Money" color="bg-emerald-600"
          value={`$${myMoneyBalance.toLocaleString()}`}
          sub="available balance"
        />
        <StatCard
          icon={TrendingUp} label="Career Tasks" color="bg-blue-600"
          value={careerTasksDone}
          sub="applications progressed"
        />
        <StatCard
          icon={BookOpen} label="Avg Skill Level" color="bg-yellow-600"
          value={`${avgSkill}%`}
          sub="finance skills"
          progress={avgSkill}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <h3 className="text-white font-semibold mb-4">Goal Completion This Week</h3>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={weekData}>
              ...
              <Tooltip
                contentStyle={{ background: "#1a1d27", border: "1px solid #ffffff10", borderRadius: "12px", color: "#e2e8f0" }}
              />
              <Area type="monotone" dataKey="goals" stroke="#6366f1" fill="url(#goalGrad)" strokeWidth={2} name="Goals %" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card lg:col-span-2">
          ...
          <div className="space-y-2">
            {useApp().todayGoals.length === 0 && (
              <p className="text-slate-500 text-sm">No goals for today. Add some in the Goals module.</p>
            )}
            {useApp().todayGoals.map(g => (
              <div key={g.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/3 border border-white/5">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${g.completed ? "bg-indigo-600 border-indigo-600" : "border-slate-600"}`}>
                  {g.completed && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                </div>
