...
import { Plus, Trash2, Sparkles, Target, Star, Check, Upload, ChevronDown, ChevronUp, ChevronLeft, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="card text-center">
      ...
      <div className="text-lg font-bold text-white">
        {value}<span className="text-sm text-slate-500">/{target}</span>
      </div>
...
export default function Nutrition() {
  ...
  // ── Date navigator (which day you're VIEWING) ──
  ...
  const viewedMeals = meals.filter(m => m.date === viewDate);

  // ── Totals for viewed date (for macro rings) ──
  const totals = viewedMeals.reduce((a, m) => ({
  ...
  const formatViewDate = () => {
    ...
    if (viewDate === yesterday.toISOString().split("T")[0]) return "Yesterday";
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // If we logged for a different date, switch view to that date
      setViewDate(form.date);
      setForm({ meal: "Breakfast", foods: "", date: form.date });
      setShowForm(false);
    } catch {
  ...
  // ── Meal plan: log planned meal to today ──
  ...
  const handleExcelImport = async (e) => {
    ...
    try {
      ...
      const plans = rows.map(row => ({
        ...
        logged: false, log_date: "",
      })).filter(p => p.foods);
      await supabase.from("meal_plans").delete().neq("id", "00000000-0000-0000-0000-000000000000");
      const { data: { user } } = await supabase.auth.getUser();
      const { data } = await supabase.from("meal_plans").insert(plans.map(p => ({ ...p, user_id: user.id }))).select();
      if (data) setMealPlans(data);
  ...
  const logFavorite = (fav) => {
    setMeals(prev => [...prev, {
      ...
      foods: fav.foods, calories: fav.calories,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          {activeTab === "Today" && (
            <button
              onClick={() => { setForm(f => ({ ...f, date: viewDate })); setShowForm(!showForm); }}
      ...
      {/* ── Macro rings (always visible, reflect viewDate on Today tab) ── */}
      ...
      {activeTab === "Today" && (
        <div className="space-y-4">
          ...
          <div className="card !py-3 !px-4 flex items-center gap-3">
            <button onClick={goBack} className="btn-secondary !p-2 flex-shrink-0">
              <ChevronLeft size={16} />
            </button>

            <div className="flex-1 flex items-center justify-center gap-3 flex-wrap">
              <div className="flex flex-col items-center">
                <span className="text-white font-semibold">{formatViewDate()}</span>
                {viewDate !== today && (
                  <span className="text-xs text-slate-500">{viewDate}</span>
                )}
              </div>
              {viewDate !== today && (
                <button onClick={goToToday} className="text-xs text-indigo-400 hover:text-indigo-300 underline">
                  Back to today
                </button>
              )}
            </div>

            {/* Also allow picking a date directly */}
            <input
              type="date"
          ...
          {showForm && (
            <div className="card border-indigo-500/20">
              <h3 className="text-white font-semibold mb-4">
                Log Meal
                {viewDate !== today && (
                  <span className="text-sm font-normal text-slate-400 ml-2">for {formatViewDate()}</span>
                )}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <CustomSelect
                  value={form.meal}
                  onChange={v => setForm({ ...form, meal: v })}
                  options={mealTypes}
                />
                <input
                  type="date"
                ...
                <div className="md:col-span-2">
                  <textarea
                    ...
                    rows={3}
                    placeholder="Describe your meal (e.g. 4 eggs, 2 slices of toast, 250ml milk)"
                    value={form.foods}
              ...
              <div className="flex gap-2 mt-3">
                <button
                  ...
                  disabled={analyzing || !form.foods.trim()}
          ...
          {/* ── Meals list for viewed date ── */}
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-white font-semibold">
                {viewDate === today ? "Today's Meals" : `${formatViewDate()}'s Meals`}
              </h3>
              <span className="text-xs text-slate-500">
                {viewedMeals.length} meal{viewedMeals.length !== 1 ? "s" : ""} · {totals.calories} cal
              </span>
            </div>
            ...
            <div className="space-y-3">
              {viewedMeals.length === 0 && (
                <div className="card text-center text-slate-500 py-10">
                  <p className="mb-2">No meals logged for {formatViewDate().toLowerCase()}.</p>
                  <button
                    onClick={() => { setForm(f => ({ ...f, date: viewDate })); setShowForm(true); }}
              ...
              {viewedMeals.map(m => (
                <div key={m.id} className="card">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button onClick={() => handleDelete(m.id)} className="text-slate-600 hover:text-red-400 flex-shrink-0">
                      <Trash2 size={15} />
                    </button>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-slate-500 text-xs mt-1">
                  Excel columns needed: Week, Day, Meal, Foods, Calories, Protein, Carbs, Fat
                </p>
              </div>
              ...
              <div key={dayNum} className="card">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">
                        {dayCalories} cal · {dayProtein}g protein · {dayPlans.length} meals
                      </div>
                    </div>
                  </div>
                ...
                {expandedDay === dayNum && (
                  <div className="mt-4 space-y-2 border-t border-white/5 pt-4">
                    {dayPlans.map(plan => (
                      <div key={plan.id}
                        className={`flex items-center justify-between p-3 rounded-xl border ${plan.logged && plan.log_date === today ? "bg-green-500/5 border-green-500/20" : "bg-white/3 border-white/5"}`}>
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className="badge bg-indigo-500/20 text-indigo-400 text-xs">{plan.meal_type}</span>
                            {plan.logged && plan.log_date === today && (
                              <span className="text-green-400 text-xs">✓ Eaten today</span>
                            )}
                          </div>
