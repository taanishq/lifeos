...
export function AppProvider({ children }) {
  ...
  const [myMoneyBalance, setMyMoneyBalanceState] = useState(5000);
  const [nutritionTargets, setNutritionTargetsState] = useState({ calories: 2800, protein: 180, carbs: 300, fat: 80 });
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      // Load user preferences from their metadata
      const balance = user.user_metadata?.myMoneyBalance ?? 5000;
      const targets = user.user_metadata?.nutritionTargets ?? { calories: 2800, protein: 180, carbs: 300, fat: 80 };
      setMyMoneyBalanceState(balance);
      setNutritionTargetsState(targets);
      ...
      setSkillsState(skillsData?.length ? skillsData : []);
      setContactsState(contactsData || []);
      setTransactionsState((txData || []).map(t => ({ ...t, usedMyMoney: t.used_my_money })));
      setJournalEntriesState((journalData || []).map(e => ({ ...e, wentWell: e.went_well, couldBeBetter: e.could_be_better, freeForm: e.free_form })));
  ...
  const initSkillsForUser = async (uid) => {
    const defaultSkills = [
      { name: "Accounting", progress: 0, hours: 0, user_id: uid },
      { name: "DCF Modeling", progress: 0, hours: 0, user_id: uid },
      { name: "LBO Modeling", progress: 0, hours: 0, user_id: uid },
      { name: "Comparable Company Analysis", progress: 0, hours: 0, user_id: uid },
      { name: "Excel Skills", progress: 0, hours: 0, user_id: uid },
      { name: "Financial Statements", progress: 0, hours: 0, user_id: uid },
      { name: "PowerPoint Skills", progress: 0, hours: 0, user_id: uid },
      { name: "Market Research", progress: 0, hours: 0, user_id: uid },
    ];
    const defaultCerts = [
      { name: "Bloomberg Market Concepts", progress: 0, completed: false, user_id: uid },
      { name: "FMVA", progress: 0, completed: false, user_id: uid },
      { name: "Wall Street Prep", progress: 0, completed: false, user_id: uid },
    ];
    const { data: s } = await supabase.from("skills").insert(defaultSkills).select();
    const { data: c } = await supabase.from("certifications").insert(defaultCerts).select();
    if (s) setSkillsState(s);
    if (c) setCertificationsState(c);
  };
  ...
  const setGoals = async (updater) => {
    ...
    const updated = next.find(n => { const old = goals.find(g => g.id === n.id); return old && JSON.stringify(old) !== JSON.stringify(n); });
    if (updated) {
  ...
  const setTransactions = async (updater) => {
    ...
    const updated = next.find(n => { const old = transactions.find(t => t.id === n.id); return old && old.usedMyMoney !== n.usedMyMoney; });
    if (updated) {
  ...
  const setApplications = async (updater) => {
    ...
    const updated = next.find(n => { const old = applications.find(a => a.id === n.id); return old && old.status !== n.status; });
    if (updated) {
  ...
  const setSkills = async (updater) => {
    ...
    const updated = next.find(n => { const old = skills.find(s => s.id === n.id); return old && old.progress !== n.progress; });
    if (updated) {
  ...
  const setCertifications = async (updater) => {
    ...
    const updated = next.find(n => { const old = certifications.find(c => c.id === n.id); return old && old.progress !== n.progress; });
    if (updated) {
  ...
  const setNutritionTargets = (val) => {
    setNutritionTargetsState(val);
    supabase.auth.updateUser({ data: { nutritionTargets: val } });
  };
